...
- 🧪 **Surveillance data & drift** : le monitoring synthétique expose maintenant les alertes qualité (sévérité, statut, action recommandée) et une analyse PSI du drift avec priorisation visuelle des variables, pour sécuriser l'ouverture aux flux temps réel.
- Nous poursuivons cette instrumentation en ajoutant `place_probability_quality`, qui calcule Brier score, log loss, erreurs absolues et met en avant les courses/pronostics sur- ou sous-confiants pour piloter la fiabilité top 3.
- Nous ajoutons une lecture complémentaire avec `place_probability_distribution`, qui caractérise la dispersion des probabilités de place, souligne l'écart gagnants/perdants et met en évidence les cas atypiques à investiguer.
- Nous enrichissons cette analyse avec `place_probability_calibration`, qui regroupe les probabilités de place par bandes de 10 points pour comparer probabilité moyenne et taux observé, et identifier immédiatement les segments sur- ou sous-calibrés.
- Nous ajoutons une courbe dédiée `place_probability_gain_curve`, qui mesure la capture cumulée des arrivées top 3 lorsque l'on retient les meilleures probabilités de place et pointe les réunions exemplaires ou à surveiller.
- Nous allons plus loin avec `place_probability_thresholds`, qui évalue plusieurs seuils de sélection, extrait les recommandations (meilleur F1, précision, rappel) et illustre les courses clés pour guider les arbitrages.
- Nous prolongeons la famille avec `place_probability_precision_recall`, qui trace la courbe précision/rappel dédiée aux pronostics de place, identifie le seuil optimal, met en avant les segments haute précision et fournit des exemples concrets pour ajuster les stratégies terrain.
- Nous complétons cette instrumentation avec `place_probability_roc`, qui échantillonne la courbe ROC des probabilités de place, calcule l'AUC, sélectionne le meilleur seuil via l'indice de Youden et fournit des exemples concrets pour illustrer les compromis entre rappel et spécificité.
- Nous enrichissons la lecture de séparation avec `place_probability_ks`, qui calcule la statistique de Kolmogorov-Smirnov, met en avant les courses les plus discriminantes et fournit des exemples gagnants/perdants pour vérifier que le modèle distingue bien les outsiders des candidats placés.
- Nous complétons cette série avec `place_probability_sharpness`, qui calcule l'écart-type des probabilités de place par course, classe les réunions selon leur dispersion et met en avant les contextes stables versus volatils pour ajuster les stratégies de sélection.
- Nous introduisons désormais la synthèse `feature_contribution_summary`, qui agrège les contributions SHAP du modèle, calcule l'influence moyenne/absolue et classe les facteurs expliquant le mieux les décisions récentes.
- Nous intégrons en parallèle le coefficient `cohen_kappa`, complémentaire au `matthews_correlation`, afin de mesurer l'accord réel entre prédictions et arrivées tout en neutralisant l'accord attendu par hasard.
- Nous complétons cette itération par le tableau `prediction_outcome_performance`, qui décompose la répartition des vrais/faux positifs et négatifs avec la part de trafic, la calibration moyenne et le rappel segmenté pour hiérarchiser les axes d'amélioration du modèle.
- Nous ajoutons désormais `probability_margin_performance` qui segmente la marge entre le favori et son dauphin pour comparer la fiabilité du modèle lorsque le premier choix dispose d'un large matelas ou lorsqu'il est contesté par un challenger proche.
- Nous complétons cette itération avec le tableau `favourite_alignment_performance`, qui compare systématiquement le favori du modèle à celui du PMU pour suivre les taux d'alignement, la réussite respective de chaque favori, les écarts de probabilité et les positions relatives dans le classement.
- Nous intégrons à présent la synthèse `winner_rank_metrics`, qui calcule le rang moyen, la médiane et le Mean Reciprocal Rank des vainqueurs ainsi que leur distribution (Top 1/Top 3) pour qualifier finement la hiérarchie du modèle.
- Nous complétons cette analyse par le tableau `topn_performance`, qui mesure course par course la couverture et le taux de gagnants capturés par les paniers Top 1 à Top 5, tout en retraçant les probabilités moyennes et les meilleures positions observées pour chaque taille de sélection.
- Nous ajoutons maintenant la synthèse `rank_error_metrics`, qui calcule l'erreur absolue moyenne, la dispersion (RMSE), la part de pronostics parfaitement classés et le biais signé entre le classement du modèle et l'arrivée officielle, course par course comme en agrégé.
- Nous étendons cette lecture avec l'histogramme `rank_error_distribution`, qui répartit les écarts de classement en bandes métier (exact, ±1 rang, 2-3 rangs, 4-5 rangs, ≥6 rangs) afin de repérer immédiatement les scénarios où la hiérarchie probabiliste décroche et d'en mesurer l'impact (part de trafic, gagnants captés, biais signé).
- Nous poursuivons cette lecture probabiliste avec `probability_distribution_metrics`, qui compare la distribution des scores entre pronostics gagnants et perdants, calcule moyennes, médianes, percentiles et écarts-types, et mesure l'écart moyen/médian entre ces deux groupes pour détecter rapidement un recouvrement anormal malgré des taux globaux stables.
- Nous consolidons désormais le suivi longitudinal avec la vue `year_performance`, qui regroupe les métriques par année civile (volumes, précision, rappel, taux observé, part de trafic, dates extrêmes) pour détecter les dérives structurelles à l'échelle d'une saison complète.
- Nous ajoutons désormais les métriques globales `ndcg_at_3` et `ndcg_at_5`, qui mesurent la qualité du classement probabiliste des pronostics (en tenant compte de la position des chevaux réellement placés) afin d'évaluer l'apport opérationnel du modèle lorsqu'on sélectionne les Top 3 ou Top 5.
- Nous introduisons maintenant la synthèse `rank_correlation_performance`, qui calcule la corrélation de Spearman course par course entre le ranking du modèle et l'arrivée réelle afin de détecter les réunions où le tri probabiliste est inversé ou parfaitement aligné.
- Nous venons d'ajouter `horse_sire_performance`, qui regroupe les résultats par lignée paternelle pour mesurer l'impact des étalons dominants sur la précision, suivre la part de trafic par famille et identifier les alliances entraîneur/éleveur qui convertissent le mieux.
- Nous étendons désormais la cartographie généalogique avec `horse_dam_performance`, qui agrège les performances par lignée maternelle, expose les parts de trafic et met en évidence les familles les plus régulières ou volatiles.
- 🧮 La nouvelle vue `prize_per_runner_performance` mesure la dotation moyenne par partant et révèle si le modèle reste fiable lorsque les allocations augmentent ou se contractent d'un meeting à l'autre.
- Nous venons en supplément d'introduire la vue `probability_edge_performance` qui segmente l'écart modèle/marché en bandes lisibles afin de suivre, pour chaque tranche d'avantage ou de déficit face aux cotes PMU, la précision réelle, la part de trafic et les marges moyennes observées, ainsi que la vue `quarter_performance` pour condenser l'évolution saisonnière par trimestre et faciliter l'analyse des meetings hiver/printemps/été/automne.
- Nous affinons désormais le diagnostic en ajoutant le tableau `probability_error_performance`, qui classe les écarts absolus entre probabilité annoncée et résultat observé en tranches métiers (≤10 pts, 10-20 pts, 20-35 pts, 35-50 pts, >50 pts) afin de repérer immédiatement les segments de pronostics à investiguer et la part de trafic associée.
- Nous complétons cette itération avec les tableaux `jockey_experience_performance` et `trainer_experience_performance`, qui regroupent les résultats par volume de courses disputées (débutants, en progression, confirmés, experts, vétérans) pour mesurer la stabilité du modèle face à des profils très expérimentés ou plus novices et exposent part de trafic, précision, rappel et métriques d'échantillonnage.
...
- **Tableau de saisonnalité trimestrielle** : la nouvelle vue `quarter_performance` regroupe les mêmes indicateurs par trimestre civil (T1 à T4) pour suivre l'efficacité du modèle sur les grands meetings saisonniers (hiver, printemps, été, automne) et prioriser les recalibrages lorsque la dynamique change.
...
- **Baromètre d'expérience des intervenants** : les nouvelles vues `jockey_experience_performance` et `trainer_experience_performance` ventilent précision, rappel, part de trafic, nombre de courses couvertes et volume d'acteurs selon des bandes de carrière (débutant, en progression, confirmé, expert, vétéran) pour contrôler la stabilité du modèle face à des profils très chevronnés ou encore en rodage.
...
- **Analyse de dotation par partant** : la vue `prize_per_runner_performance` calcule la récompense moyenne par cheval au départ, expose précision, rappel, part de trafic et taille des pelotons pour détecter les contextes où la rémunération individuelle influe sur la qualité des pronostics.
...
- **Analyse par robe du cheval** : le tableau `horse_coat_performance` ventile la précision, le rappel, la part de trafic et le taux observé selon la couleur officielle (bai, alezan, gris, etc.) tout en listant des exemples de libellés bruts pour faciliter les rapprochements terrain.
...
- **Analyse Top N cumulée** : la vue `topn_performance` mesure, pour chaque panier Top 1 à Top 5, la couverture de courses, les taux de gagnants/podiums capturés ainsi que les probabilités moyennes et positions finales typiques afin de vérifier que l'élargissement de la sélection améliore réellement la fiabilité opérationnelle.
- **Erreur de classement moyenne** : le tableau `rank_error_metrics` synthétise l'écart absolu moyen, la médiane, le RMSE et le biais signé entre le classement probabiliste et l'arrivée réelle, tout en indiquant le nombre de pronostics parfaitement classés par course.
- **Qualité du classement (NDCG)** : les métriques `ndcg_at_3` et `ndcg_at_5` quantifient à quel point les gagnants/podiums sont bien positionnés dans le classement probabiliste du modèle, ce qui aide à piloter les stratégies basées sur une sélection des Top 3 ou Top 5 chevaux.
...
- **Analyse par tranche de température** : le tableau `temperature_band_performance` consolide précision, rappel, part de trafic et statistiques (moyenne, médiane, min/max) pour chaque bande de température (gel, très froid, frais, tempéré, chaud, caniculaire) afin de vérifier si le modèle reste stable lorsque les réunions se déroulent dans des conditions extrêmes ou simplement fraîches.
- **Analyse saisonnière** : la vue `season_performance` agrège précision, rappel, part de trafic, volume de réunions et période couverte pour chaque saison (hiver, printemps, été, automne) afin de repérer les tendances multi-mois et adapter les plans de recalibrage.
- **Analyse annuelle** : la vue `year_performance` regroupe les métriques de précision, rappel, part de trafic, volume de courses/réunions et plage de dates pour chaque année civile, offrant une lecture macro des dérives potentielles du modèle sur une saison complète.
...
- **Streamlit** : Dashboard exploratoire avancé (bankroll, ROI, value bets, météo, calibration probabiliste)
