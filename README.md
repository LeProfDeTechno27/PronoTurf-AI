...
- 🏇 **Sprint monitoring ML en cours** : nous enrichissons itérativement le tableau de bord d'évaluation du modèle. Le dernier incrément introduit les vues `horse_age_performance`, `horse_gender_performance`, `day_part_performance`, `weekday_performance`, `hippodrome_performance`, `track_type_performance`, `odds_band_performance`, `handicap_performance` et `weight_performance` pour comparer la précision selon la maturité des chevaux, leur genre, la plage horaire des réunions, le jour de la semaine, l'hippodrome, la nature de la piste, le profil de cote, la charge de handicap et le poids porté, en complément des tableaux par catégorie officielle, dotation, numéro de corde et mode de départ.
